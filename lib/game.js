...
Game.prototype.load = function () {
  ...
  for (let i = 0; i < this.characters.length; i++) {
    images.push(
      Loader.loadImage('hero', (this.characters[i].prototype || {}).image || this.characters[i].image)
    );
  }
...
Game.prototype.initMap = function () {
  ...
  if (HeroConstructor === Hero || Hero.prototype.isPrototypeOf(HeroConstructor.prototype)) {
    this.hero = new HeroConstructor(map, 160, 160);
  }
  else {
    this.hero = HeroConstructor(map, 160, 160);
  }

  document.addEventListener('keydown', e => e.keyCode === 32 && this.hero.say());
...
Game.prototype.render = function () {
  ...
  // draw main character
  const heroWidth = (typeof this.hero.width === 'function') ? this.hero.width() : this.hero.width;
  const heroHeight = (typeof this.hero.height === 'function') ? this.hero.height() : this.hero.height;

  this.ctx.drawImage(
    this.hero.image,
    this.hero.screenX - heroWidth / 2,
    this.hero.screenY - heroHeight / 2
  );
